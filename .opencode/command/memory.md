...
Tip: /memory <text> to store  |  /memory pin <topic> to pin  |  /memory unpin <topic> to unpin  |  /memory remove <topic> to remove  |  /memory consolidate to consolidate this session  |  /memory repair to re-index orphaned topic files
...
This scans the active memory directory for topic `.md` files that are missing from `MEMORY.md` and appends index entries for them (marked `[stale?]`). It never deletes or overwrites existing entries, and it skips any topic you intentionally deleted via `remove_memory`/`/memory remove` (recorded in the directory's `.ocl-removed` list), so deliberate removals are never resurrected. Useful when another tool sharing the directory (e.g. openpi-memory) wrote topic files without updating the shared index.
