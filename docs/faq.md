...
- The cross-process lock stores a `pid\tts\trand` token and only reclaims a >10s-old lock after a `process.kill(pid, 0)` liveness check (a foreign/unparseable payload is reclaimed only past 60s) *and* a re-stat confirming the lock is still the same file it inspected — so a live but slow holder is never stolen from, and two simultaneous reclaimers cannot clobber each other's fresh lock. Release is compare-and-delete: a writer never unlinks a lock that was stolen from it mid-operation. The `wx`-flag atomicity may still not hold on some older NFS-mounted home directories — an accepted trade-off for a single-user lock design, not a bug with a planned fix (for now).
...
Discovery is index-driven: the agent only sees topics listed in `MEMORY.md`. A co-tenant tool (e.g. openpi-memory) can write topic files, or rewrite the shared `MEMORY.md` from its own smaller view, leaving your topic files present on disk but absent from the index — undiscoverable. Run `/memory repair`: it scans the active dir for topic `.md` files missing from the index and appends an entry for each (marked `[stale?]`, dated from the file's own frontmatter). It is additive-only — never deletes, reorders, or overwrites — and idempotent. When `shared_dir` is active and more than 5 files are unindexed (intentionally-removed files excluded), the injected memory block shows a maintenance note reminding you to run it. Repair is never automatic (a co-tenant may be mid-write), so it is always your explicit call. **It will not resurrect a topic you intentionally deleted:** `remove_memory` records the removed filename in a `.ocl-removed` list inside the memory dir, and repair skips anything on that list — openpi-memory maintains the same-named list, so under `shared_dir` either tool's removal is honored by both. If you later `write_memory` that same topic again, it reclaims the existing file and clears the tombstone, so the choice to restore is yours, never repair's.
