...
| `ocl-memory-shared.mjs` | Pure-logic module with no plugin hooks — path/config resolution (`getMemoryDir`, `getMemoryIndex`, `getDirtySentinel`), config parsing (`readMemoryRules`, `parseRules`, string-literal-aware `stripJsonc`), a filename safety check (`isSafeFilename`, rejects path separators and `..` segments in filenames read back from `MEMORY.md`), file I/O (`atomicWriteFileSync`, `ensureMemoryDir`), the cross-process lock (`acquireLock`/`releaseLock`, PID-stamped with a `process.kill(pid,0)` liveness check before stale reclaim), the single `withLock(memDir, fn, {strict})` mutation entry point (fail-closed with one auto-retry under `shared_dir`, best-effort locally), `LockContendedError`, and the one-time `shared_dir` carry-over. Identical pre-existing topic files still gain a missing shared index entry. Imported by both `ocl-memory.mjs` and `ocl-memory-tui.mjs` so both plugins resolve the exact same active directory and share identical lock semantics. |
...
As of v0.6.4, shared-dir writes fail closed on lock contention (auto-retry once, then a busy message) instead of proceeding unlocked, the lock is PID-stamped with a liveness check before stale reclaim, a `repair_memory` tool / `/memory repair` command re-indexes topic files a co-tenant left unindexed, new topic frontmatter is quoted to match openpi-memory, and the consolidation recap topic was renamed to `ocl-last-session-recap.md` so openpi-memory's recap-retirement cannot strip it. The same release also scopes `write_memory`'s append-mode `last_updated` bump to the frontmatter block only (never a body line), stops `readMemoryIndex` from creating `MEMORY.md` on the unlocked read path (it is now created only by the locked tool/carry-over paths), and guards the TUI's `setPin`/`removeEntry` against a missing index. Intentional removals are now remembered: `remove_memory` writes the filename to a tool-private `.ocl-removed` tombstone list, `repair_memory` skips tombstoned files (so a deliberate removal is never resurrected as a co-tenancy "orphan"), and re-writing a tombstoned topic reclaims its on-disk file and clears the tombstone instead of bumping a collision suffix.

As of v0.6.5, the index's flatfile contract (one topic = one line) is enforced against markdown itself: `sanitizeIndexField` collapses newlines and strips `[]()` from a topic name or summary before it reaches the index line (so a crafted summary can no longer split a record or inject a phantom entry under `shared_dir`), `isSafeFilename` additionally rejects leading-dot and bracket/paren filenames at every read boundary, and `stripJsonc` now handles `/* */` block comments (string-literal-aware) so a valid-JSONC config no longer silently reverts to defaults.
...
**2. Token injection** — `MEMORY.md` and `memory.jsonc` are injected into the system prompt on:
  ...
  - Any turn immediately following a memory tool mutation (`write_memory`, `remove_memory`, `pin_memory`, `repair_memory`)

All other turns receive no injection. Users can tune `inject_every_n_turns` in `memory.jsonc` — lower values (e.g. `2`) re-emit cached state more often; higher values (e.g. `10`) reduce token overhead. Manual edits require a memory-tool mutation, compaction, or session restart to refresh the cache. In addition to this, the tool injects current `MEMORY.md` and rendered `memory.jsonc` content into the compaction context so memory survives context compression cleanly.
